...
**Close stdin with `< /dev/null`.** Even when the prompt is passed as a positional argument, `codex exec` reads *additional* input from stdin and will hang indefinitely (`Reading additional input from stdin...`) if stdin is an open pipe rather than a TTY. Non-interactive invocations must redirect stdin from `/dev/null`.

**Runtime.** A real Codex run routinely takes several minutes. Run it with a generous timeout (or in the background) rather than a short one.

Always pass `-m gpt-5.6-sol` — implementation work runs on gpt-5.6-sol regardless of the user's Codex defaults.

**Reasoning effort.** Default to `-c model_reasoning_effort="medium"`. Use `"low"` for trivial, mechanical work (small renames, obvious one-liners, boilerplate). Use `"high"` for hard, complex work (subtle logic, cross-cutting refactors, tricky concurrency or state). Never use a reasoning level higher than `"high"`.
